...
(function ($) {
    ...
    $.fn.comboboxfilter.methods = {
        ...
        init: function (target, ops) {
            ...
            if (options.unlimit) {//如果开启不限 则添加,unlimitText(此时里面的内容为'不限')
...
//                var anyNodeCopy = $('<ul class="any"><li><a class="filter-unlimit filter-tag selected" href="javascript:;" data-value="">' + options.unlimitText + '</a></li></ul>').bind('click',function() {
//                    $(anyNode).find('.filter-unlimit').removeClass('selected').addClass('selected');
//                    $this.clear(target);
//                });
                listcontainer.append(anyNode);
//                listcontainer.append(anyNodeCopy);
            }
            ...
            $(target).html($('<strong class="tit1">'+options.title_text+' </strong>'));
            ...
            if (options.scope) {
                ...
                if ($('#' + options.scope).length>0) {
                    ...
                    var node = $('<div id="' + options.scope + '" class="hotel-filter-list "><strong class="tit">已选条件：</strong><div class="con selected-query"><ul  class="list"><li class="filter-query-clear"><a class="J_FilterQueryClear" href="javascript:;">全部清除</a></li></ul></div></div>');
        ...
        getText:function(target){
        	var selectText = $(target).find('.list .selected');
        	var arrText = new Array();
        	$.each(selectText,function(i,item){
        		arrText.push($(item).attr('data-text'));
        	});
        	return arrText.join(",");
        },
        ...
        reSetValue: function (target) {
            ...
            this.options(target).onChange(value,text);//text是我自己填的
        },
        clear: function (target) {
        	$('.filter-query-clear').siblings().remove();//点击清楚全部
